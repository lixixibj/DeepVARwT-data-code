...
def make_var_cov_matrix_for_innovation_of_varp_numpy(lower_triang_params, m, order):
    """
...
def A_coeffs_for_causal_VAR_numpy(A_coeffs_from_lstm, p, d, var_cov_innovations_varp):
    """
...
def get_A_coeff_m_for_VAR_1_numpy(inital_A_m, m, order):
    """
...
def make_var_cov_of_innovations_var1_numpy(lower_triang_params_form_lstm, m, order):
    """
...
def calculate_p0_numpy(m, order, A_coeffs_var1, var_cov_innovations_var1):
    """
    Calculate variance-covariance matrix of (y_b,...,y_1)
    """
    i_matrix = np.eye(m * m * order * order)
    i_F_F = i_matrix - kron_numpy(A_coeffs_var1, A_coeffs_var1)
...
def transfrom_var_cov_matrix_numpy(var_cov_matrix, order, m):
...
def get_lagged_observations_numpy(target_y, current_t, order, m):
    """
...
def compute_log_likelihood_numpy(target, var_coeffs, residual_parameters, m, order):
    """
    ...
    var_cov_innovations_varp = make_var_cov_matrix_for_innovation_of_varp_numpy(residual_parameters, m, order)
    all_casual_coeffs = A_coeffs_for_causal_VAR_numpy(var_coeffs, order, m, var_cov_innovations_varp)
    A_coeffs_var1 = get_A_coeff_m_for_VAR_1_numpy(all_casual_coeffs, m, order)
    A = put_A_coeff_together(all_casual_coeffs, order)
    var_cov_innovations_var1 = make_var_cov_of_innovations_var1_numpy(residual_parameters, m, order)
    
    # calculate variance-covariance matrix of first p obs (y_1,...,y_p)
    var_cov_matrix_for_initial_p_obs = calculate_p0_numpy(m, order, A_coeffs_var1, var_cov_innovations_var1)
    var_cov_matrix_for_initial_p_obs_update = transfrom_var_cov_matrix_numpy(var_cov_matrix_for_initial_p_obs, order, m)
    ...
    for t in range(order, len_of_seq):
        lagged_obs = get_lagged_observations_numpy(target, t, order, m)
...
def objective_function_numpy(params, data, m, order):
    """
    ...
    try:
        ...
        neg_log_likelihood = compute_log_likelihood_numpy(data, ar_params, residual_params, m, order)
...
def optimize_params_numpy(data, iters, m, order, res_saving_path):
    """
    ...
    var_cov_innovations_varp = make_var_cov_matrix_for_innovation_of_varp_numpy(final_residual_params, m, order)
    all_coeffs = A_coeffs_for_causal_VAR_numpy(final_ar_params, order, m, var_cov_innovations_varp)
...
def process_section(num, data_saving_path, iters, m, order, res_saving_path):
    ...
    try:
        np.random.seed(num)
        all_coeffs, var_cov_m = optimize_params_numpy(data, iters, m, order, res_saving_path1)
    except Exception as e:
