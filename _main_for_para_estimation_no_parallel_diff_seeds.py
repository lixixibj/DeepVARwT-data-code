...
res_saving_path = './simulation-study/100-res-010925-diff-seeds/'
...
for num in range(exps):
    print(f"Running experiment {num+1}/{exps}")
    result = process_section(num, data_saving_path, num_layers, hidden_dim, iter1, iter2,
                             lr, lr_trend, m, order, res_saving_path, threshould)
...
# Save results
pd.DataFrame(coeffs_all).to_csv('./simulation-study/100-res-010925-diff-seeds/all_coeffs_100exps_010925_diff_seeds.csv')
pd.DataFrame(var_cov_all).to_csv('./simulation-study/100-res-010925-diff-seeds/all_var_cov_100exps_010925_diff_seeds.csv')
